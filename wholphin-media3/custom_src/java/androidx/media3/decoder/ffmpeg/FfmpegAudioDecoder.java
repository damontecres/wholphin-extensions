 ...
 * https://github.com/NuvioMedia/NuvioTV/tree/060554f/ffmpeg-decoder-downmix
 * Provenance details:
 * https://github.com/NuvioMedia/NuvioTV/blob/060554f/ffmpeg-decoder-downmix/NOTICE.md
 */
/*
...
/* package */ final class FfmpegAudioDecoder
  ...
  // AC3 transcoding position tracking
  private long ac3OutputPositionUs;
  private boolean ac3PositionAnchored;
  ...
  protected FfmpegDecoderException decode(
    ...
    if (reset) {
      ...
      ac3OutputPositionUs = 0;
      ac3PositionAnchored = false;
    }
    // For AC3 transcoding, anchor output timestamps to the input PTS so that
    // ExoPlayer's position tracking stays on the same timeline as the source.
    // If we started at 0 while the source PTS is ~1e12 µs (Jellyfin offset),
    // ExoPlayer would compute currentPosition ≈ -1,000,000 seconds and stop loading.
    if (outputEncoding == C.ENCODING_AC3 && !ac3PositionAnchored) {
      ac3OutputPositionUs = (inputBuffer.timeUs != C.TIME_UNSET) ? inputBuffer.timeUs : 0;
      ac3PositionAnchored = true;
    }
    ...
    if (result == AUDIO_DECODER_ERROR_OTHER) {
      Log.e("FfmpegAudioDecoder", "Error decoding (see ffmpeg_jni log for details)");
    ...
    // For AC3 transcoding, compute correct timestamps based on accumulated output position
    // rather than using the input packet PTS, since AC3 frame boundaries don't align with
    // input packet boundaries (due to the resampling + fifo buffering).
    if (outputEncoding == C.ENCODING_AC3) {
      outputData = checkNotNull(outputBuffer.data);
      int ac3FrameCount = countAc3Syncframes(outputData, result);
      outputBuffer.timeUs = ac3OutputPositionUs;
      long advance =
          ac3FrameCount > 0 && sampleRate > 0
              ? (long) ac3FrameCount * 1536L * 1000000L / sampleRate
              : result * 1000000L / 80000;
      ac3OutputPositionUs += advance;
    }
  ...
  private static int countAc3Syncframes(ByteBuffer buffer, int size) {
    ByteBuffer readBuffer = buffer.duplicate();
    int start = readBuffer.position();
    int limit = Math.min(readBuffer.limit(), start + size);
    int offset = start;
    int frameCount = 0;
    byte[] header = new byte[6];
    while (offset + header.length <= limit) {
      readBuffer.position(offset);
      readBuffer.get(header);
      int frameSize = Ac3Util.parseAc3SyncframeSize(header);
    ...
    return offset == limit ? frameCount : 0;
  }
