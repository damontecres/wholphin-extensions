...
public final class FfmpegAudioRenderer extends DecoderAudioRenderer<FfmpegAudioDecoder> {
  ...
  public FfmpegAudioRenderer(
      ...
      @Nullable AudioRendererEventListener eventListener,
      AudioSink audioSink) {
    super(eventHandler, eventListener, new Ac3FrameCountingAudioSink(audioSink));
  }
  ...
  protected @C.FormatSupport int supportsFormatInternal(Format format) {
    ...
    if (transcodeToAc3) {
      int sampleRate = resolveAc3OutputSampleRate(format.sampleRate);
  ...
  private static int resolveAc3OutputSampleRate(int inputSampleRate) {
    return inputSampleRate == 32000 || inputSampleRate == 44100 ? inputSampleRate : 48000;
  }

  private static final class Ac3FrameCountingAudioSink extends ForwardingAudioSink {
    private boolean isAc3;

    public Ac3FrameCountingAudioSink(AudioSink sink) {
      super(sink);
    }
    ...
    public void configure(
      ...
      isAc3 = MimeTypes.AUDIO_AC3.equals(inputFormat.sampleMimeType);
      super.configure(inputFormat, specifiedBufferSize, outputChannels);
    }

    @Override
    public boolean handleBuffer(
        java.nio.ByteBuffer buffer, long presentationTimeUs, int encodedAccessUnitCount)
        throws InitializationException, WriteException {
      if (isAc3 && encodedAccessUnitCount == 1 && buffer != null && buffer.remaining() >= 6) {
        int frameCount = countAc3Syncframes(buffer);
        if (frameCount > 0) {
          encodedAccessUnitCount = frameCount;
        }
      }
      return super.handleBuffer(buffer, presentationTimeUs, encodedAccessUnitCount);
    }

    private static int countAc3Syncframes(java.nio.ByteBuffer buffer) {
      java.nio.ByteBuffer readBuffer = buffer.duplicate();
