    ...
    if (decoderContext->transcode_to_ac3) {
      ...
      while (av_audio_fifo_size(decoderContext->fifo) >= decoderContext->encoder_context->frame_size) {
        ...
        if (ret == AVERROR(EAGAIN) || ret == AVERROR_EOF) {
          // Encoder needs more data (codec delay); keep the frame, skip output
...
int configureResampler(DecoderContext* decoderContext, AVFrame* frame,
  ...
  if (decoderContext->transcode_to_ac3) {
    // Explicit 5.1 layout
    outputLayout = AV_CHANNEL_LAYOUT_5POINT1_BACK;
    decoderContext->output_sample_format = AV_SAMPLE_FMT_FLTP;
    // AC3 supports only 32000, 44100, and 48000 Hz
    if (outputSampleRate == 44100 || outputSampleRate == 32000) {
      // keep as-is, these are valid
    } else {
