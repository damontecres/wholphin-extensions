...
SCRIPTS_PATH="$(realpath $SCRIPT_PATH/scripts)"
BUILD_PATH="$(realpath ../../$SCRIPT_PATH/build/libmpv)"
DEPS_PATH="$(realpath ../../$SCRIPT_PATH/build/libmpv/deps)"
cd "$SCRIPT_PATH"
. ./include/depinfo.sh

export SCRIPT_PATH="$(realpath $SCRIPT_PATH)"
...
loadarch () {
	...
	export prefix_dir="$BUILD_PATH/prefix/$prefix_name"
	if [ $clang -eq 1 ]; then
...
build () {
	if [ $1 != "mpv-android" ] && [ ! -d "$DEPS_PATH/$1" ]; then
